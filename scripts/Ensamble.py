...
y_pred1 = classifier1.predict_proba(X_train1)

classifier2 = RandomForestClassifier(n_estimators=200, max_depth= 15)  
classifier2.fit(X_train2, y_train) 
y_pred2 = classifier2.predict_proba(X_train2)

classifier3 = RandomForestClassifier(n_estimators=200, max_depth= 15)  
classifier3.fit(X_train3, y_train) 
y_pred3 = classifier3.predict_proba(X_train3)

classifier4 = RandomForestClassifier(n_estimators=200, max_depth= 15)  
classifier4.fit(X_train4, y_train) 
y_pred4 = classifier4.predict_proba(X_train4)

###armo el df para entrenamiento de ensamble

df_ensamble=pd.DataFrame({'Column1':y_pred1[:,1],'Column2':y_pred2[:,1],'Column3':y_pred3[:,1],'Column4':y_pred4[:,1]})

###entreno modelo final
classifier_final = RandomForestClassifier(n_estimators=1, max_depth= 2)
classifier_final.fit(df_ensamble, y_train) 

###genero probs. en test
X_test_prob1=classifier1.predict_proba(X_test1)
X_test_prob2=classifier2.predict_proba(X_test2)
X_test_prob3=classifier3.predict_proba(X_test3)
X_test_prob4=classifier4.predict_proba(X_test4)

###armo el df para test ensable

df_ensamble_test=pd.DataFrame({'Column1':X_test_prob1[:,1],'Column2':X_test_prob2[:,1],'Column3':X_test_prob3[:,1],'Column4':X_test_prob4[:,1]})


###evaluo modelo en train
y_pred = classifier_final.predict_proba(df_ensamble)
fpr, tpr, thresholds = metrics.roc_curve(y_train, y_pred[:,1])
metrics.auc(fpr, tpr)

###evaluo modelo en test
y_pred = classifier_final.predict_proba(df_ensamble_test)
